...
test("store: снапшот пишется в tasks-state.json и восстанавливается на старте", async () => {
  const bus = createEventBus();
  const config = makeConfig();
  const farm = createFarm(config, createSimRunners(), bus);
  const store = createTaskStore({ bus, config, runQueueTask: (spec) => farm.runTask(spec) });
  ...
  assert.ok(fs.existsSync(stateFile), "tasks-state.json должен существовать");
