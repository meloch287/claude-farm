...
В режиме **Ультракод** перед вердиктом Sniffer'а в Теплице запускаются до 8 **параллельных субагентов** на последнем Sonnet (`claude-sonnet-4-6`) по умолчанию. Каждый получает свой тип проверки, типы циклически распределяются по количеству:
