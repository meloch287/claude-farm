...
//
// Task record:
//   {id, title, input, source: "manual"|"ai-parent"|"subtask"|"demo",
//    parentId?, status: "queued"|"splitting"|"split"|"kitchen"|"corridor"|
//    "living"|"bath"|"done"|"failed", attempts, lastMessage, createdAt,
//    config?, finishedAt?}
//
// This module also owns the GLOBAL SETTINGS (engine Клауд/Кодекс, models,
// ultracode mode, subagents) persisted at output/settings.json, and the
// per-task config merge: POST /api/task may carry a partial config that is
// merged over the global settings and stored on the task.
...
const STATE_FILE_NAME = 'tasks-state.json';
const DECOMPOSE_TIMEOUT_MS = 90_000;
const CLI_PROBE_TIMEOUT_MS = 5_000;
const EXECUTORS = new Set(['sim', 'claude', 'codex']);
...
/** lastMessage note when the codex executor falls back to simulation. */
export const CODEX_FALLBACK_NOTE = 'codex CLI недоступен — выполнено симуляцией';

// ---------------------------------------------------------------------------
// Global settings: engine, models, ultracode subagents. Persisted as
...
export const CODEX_SPEEDS = ['normal', 'faster'];
 ...
 * @param {object} [raw]
 * @returns {{engine: string, claude: object, codex: object}}
 */
export function normalizeSettings(raw) {
  const src = raw && typeof raw === 'object' ? raw : {};
  const claude = src.claude && typeof src.claude === 'object' ? src.claude : {};
  const codex = src.codex && typeof src.codex === 'object' ? src.codex : {};
  const sub = claude.subagents && typeof claude.subagents === 'object' ? claude.subagents : {};
  const d = DEFAULT_SETTINGS;
  return {
    engine: ENGINES.includes(src.engine) ? src.engine : d.engine,
    claude: {
      model: isNonEmptyString(claude.model) ? claude.model.trim() : d.claude.model,
      mode: CLAUDE_MODES.includes(claude.mode) ? claude.mode : d.claude.mode,
      subagents: {
        model: isNonEmptyString(sub.model) ? sub.model.trim() : d.claude.subagents.model,
        count: isValidCount(sub.count) ? sub.count : d.claude.subagents.count,
        types: sanitizeTypes(sub.types) ?? [...d.claude.subagents.types],
      },
    },
    codex: {
      model: isNonEmptyString(codex.model) ? codex.model.trim() : d.codex.model,
      speed: CODEX_SPEEDS.includes(codex.speed) ? codex.speed : d.codex.speed,
    },
  };
}
...
export function validateSettingsPatch(patch) {
  ...
  if ('engine' in patch && !ENGINES.includes(patch.engine)) {
    errors.push('Поле engine должно быть "claude" или "codex"');
  }
  if ('claude' in patch) {
    const claude = patch.claude;
    if (!claude || typeof claude !== 'object' || Array.isArray(claude)) {
      errors.push('Поле claude должно быть объектом');
    } else {
      if ('model' in claude && !isNonEmptyString(claude.model)) {
        errors.push('Поле claude.model должно быть непустой строкой');
      }
      if ('mode' in claude && !CLAUDE_MODES.includes(claude.mode)) {
        errors.push('Поле claude.mode должно быть "ultracode" или "normal"');
      }
      if ('subagents' in claude) {
        const sub = claude.subagents;
        if (!sub || typeof sub !== 'object' || Array.isArray(sub)) {
          errors.push('Поле claude.subagents должно быть объектом');
        } else {
          if ('model' in sub && !isNonEmptyString(sub.model)) {
            errors.push('Поле claude.subagents.model должно быть непустой строкой');
          }
          if ('count' in sub && !isValidCount(sub.count)) {
            errors.push(`Поле claude.subagents.count должно быть целым числом от 0 до ${SUBAGENT_CAP}`);
          }
          if ('types' in sub
            && !(Array.isArray(sub.types) && sub.types.every((t) => SUBAGENT_TYPES.includes(t)))) {
            errors.push(`Поле claude.subagents.types — массив из: ${SUBAGENT_TYPES.join(', ')}`);
          }
        }
      }
    }
  ...
  if ('codex' in patch) {
    ...
    if (!codex || typeof codex !== 'object' || Array.isArray(codex)) {
      errors.push('Поле codex должно быть объектом');
    } else {
      if ('model' in codex && !isNonEmptyString(codex.model)) {
        errors.push('Поле codex.model должно быть непустой строкой');
      }
      if ('speed' in codex && !CODEX_SPEEDS.includes(codex.speed)) {
        errors.push('Поле codex.speed должно быть "normal" или "faster"');
      }
    }
  }
  return errors;
...
export function createSettingsStore({ config } = {}) {
  ...
  function update(patch) {
    ...
    if ('engine' in patch) next.engine = patch.engine;
    if (patch.claude && typeof patch.claude === 'object') {
      if ('model' in patch.claude) next.claude.model = patch.claude.model;
      if ('mode' in patch.claude) next.claude.mode = patch.claude.mode;
      if (patch.claude.subagents && typeof patch.claude.subagents === 'object') {
        const sub = patch.claude.subagents;
        if ('model' in sub) next.claude.subagents.model = sub.model;
        if ('count' in sub) next.claude.subagents.count = sub.count;
        if ('types' in sub) next.claude.subagents.types = [...sub.types];
      }
    }
    if (patch.codex && typeof patch.codex === 'object') {
      if ('model' in patch.codex) next.codex.model = patch.codex.model;
      if ('speed' in patch.codex) next.codex.speed = patch.codex.speed;
    }
 ...
 * Claude tasks:  {engine:"claude", model, mode, subagents:{model,count,types}}
 * Codex tasks:   {engine:"codex", model, speed}
 * @param {object} [settings] global settings (normalized leniently)
 * @param {object} [raw] per-task config {engine, model, mode, speed, subagents}
 * @returns {object} effective task config
 */
export function mergeTaskConfig(settings, raw) {
  const base = normalizeSettings(settings);
  const src = raw && typeof raw === 'object' ? raw : {};
  const engine = ENGINES.includes(src.engine) ? src.engine : base.engine;
  if (engine === 'codex') {
    return {
      engine: 'codex',
      model: isNonEmptyString(src.model) ? src.model.trim() : base.codex.model,
      speed: CODEX_SPEEDS.includes(src.speed) ? src.speed : base.codex.speed,
    };
  }
  const sub = src.subagents && typeof src.subagents === 'object' ? src.subagents : {};
  const subBase = base.claude.subagents;
  return {
    engine: 'claude',
    model: isNonEmptyString(src.model) ? src.model.trim() : base.claude.model,
    mode: CLAUDE_MODES.includes(src.mode) ? src.mode : base.claude.mode,
    subagents: {
...
/**
 * Pick runners for a task. Pure — unit-testable without spawning any CLI.
 * Demo tasks always run on the sim. Real tasks follow the task's engine:
 * codex => codex CLI when available, claude => claude CLI when available,
 * otherwise the sim with an honest Russian fallback note.
 * @param {{source?: string, config?: object, taskConfig?: object,
 *          claudeAvailable?: boolean, codexAvailable?: boolean}} args
 * @returns {{executor: "sim"|"claude"|"codex", note?: string}}
 */
export function selectExecutor({ source, config, taskConfig, claudeAvailable, codexAvailable } = {}) {
  if (source === 'demo') {
    return { executor: 'sim' };
  }
  ...
  if (taskConfig?.engine === 'codex') {
    if (codexAvailable === true) {
      return { executor: 'codex' };
    }
    return { executor: 'sim', note: CODEX_FALLBACK_NOTE };
  }
...
/**
 * Probe a CLI (`<cmd> --version`) with a short timeout.
 * The result is cached per command for the lifetime of the process.
 * @returns {Promise<boolean>}
 */
function detectCli(cmd, { timeoutMs = CLI_PROBE_TIMEOUT_MS, force = false } = {}) {
  if (cliProbes.has(cmd) && !force) {
    return cliProbes.get(cmd);
  }
  const probe = new Promise((resolve) => {
    ...
    let child;
    try {
      child = spawn(cmd, ['--version'], { stdio: ['ignore', 'ignore', 'ignore'] });
    } catch {
      settle(false);
      return;
    }
...
/** Probe the codex CLI (`codex --version`); cached. */
export function detectCodexCli(options = {}) {
  return detectCli('codex', options);
}
 ...
 *                         executor: "sim"|"claude"|"codex") => Promise<any>,
 *          claudeAvailable?: boolean | Promise<boolean> | (() => boolean | Promise<boolean>),
 *          codexAvailable?: boolean | Promise<boolean> | (() => boolean | Promise<boolean>),
 *          getSettings?: () => object}} options
 * @returns {{createTask(args: {title: string, input?: string, mode: "simple"|"ai", source?: string, config?: object}): object,
 *            list(): object[], get(id: string): object | undefined}}
 */
export function createTaskStore({
  ...
  function persist() {
    try {
      ...
      writeFileSync(
        stateFile,
        JSON.stringify({ tasks: [...tasks.values()] }, null, 2) + '\n',
        'utf8'
      );
    } catch {
      // ignore fs errors
  ...
  // were mid-pipeline restore as queued and re-enter the FIFO queue.
  function load() {
    let snapshot;
    try {
  ...
  function pump() {
    ...
    Promise.resolve()
      .then(async () => {
        // Pick runners per task: demo => sim, otherwise the engine from the
        // task config (Клауд или Кодекс) when its CLI is available, else
        // honest sim fallback with a Russian note.
        const pick = selectExecutor({
          source: task.source,
          config,
          taskConfig: task.config,
          claudeAvailable: await resolveAvailable(claudeAvailable),
          codexAvailable: await resolveAvailable(codexAvailable),
        });
  ...
  async function decompose(parent) {
    ...
    for (const sub of subtasks) {
      const child = makeRecord({
        ...
        source: 'subtask',
        status: 'queued',
        // Subtasks inherit the parent's effective config (engine, model...).
        config: live.config,
      });
  ...
  function makeRecord({ title, input, source, status, config: rawConfig }) {
    counter += 1;
    return {
      id: 't' + counter,
      title: String(title),
  ...
  function createTask({ title, input, mode, source, config: rawConfig }) {
    const ai = mode === 'ai';
